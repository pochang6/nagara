...
版ごとに何が変わったかは [CHANGELOG.md](CHANGELOG.md) にまとめています。
...
- ただし **しばらく使われなければ静かに落とします**（一度きりの日に居座らないため）
  - 既定は15分。メニューの「しばらく使わなければ閉じる」から
    15分・30分・45分・1時間・2時間・6時間で選べます
  - この時間は**最後に1文を合成してから**の時間です。使うたびにゼロへ戻ります
  - 再生中・一時停止中は数えません
...
待ち時間の段そのものは `settings.json` の `engineIdleQuitLadder` で足し引きできます。
